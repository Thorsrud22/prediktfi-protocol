...
function mapIntentToFeedItem(v2Intent: any): LocalOverlayItem {
  ...
  const shortWallet = walletAddress.length > 8 
...
function getLocalOverlayFromV2(pubkey?: string): LocalOverlayItem[] {
  ...
  try {
    ...
    const keys = Object.keys(localStorage).filter(k =>
      k.startsWith(prefix) && (!wantedKey || k === wantedKey)
    );
...
export default function FeedPage() {
  ...
  function normalizeServerItemToFeedItem(s: any): FeedItem {
    return {
      id: s.id || s._id || s.slug || crypto.randomUUID(),
      createdAt: Number.isFinite(s.createdAt) ? s.createdAt : Date.parse(s.createdAt || new Date().toISOString()),
      title: s.title || s.question || s.text || 'Untitled',
      category: mapCategory(s.category || s.topic || s.tag),
  ...
  const merged = useMemo(() => {
    ...
    const normalizedOverlay = localOverlayItems.map(item => ({
      id: item.id,
    ...
    const allItems = [...localItems, ...normalizedOverlay, ...normalizedServer];
  ...
  const visible = useMemo(() => {
    ...
    if (showMine) {
      result = result.filter(it => it.source === 'wallet' || it.author?.handle?.includes('You'));
    }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {visible.map((insight) => (
              ...
              >
                ...
                {insight.source === 'wallet' && insight.subtitle && (
            ...
            <div className="inline-flex items-center px-6 py-3 bg-white/5 rounded-xl border border-white/10 backdrop-blur-sm">
              ...
              <span className="text-blue-200 font-medium">
                ...
                {feedData && feedData.pagination.total > 0 && (
                  <> of <span className="text-white font-bold">{feedData.pagination.total + localItems.length}</span> total</>
                )}
              </span>
            </div>
          </div>
