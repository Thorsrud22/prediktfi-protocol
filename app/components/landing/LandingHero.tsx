...
function loopTransition(enabled: boolean, duration: number, delay = 0) {
    if (!enabled) {
...
const badgeEnterVariants = makeEnterVariants(6, 0.7);
const lineEnterVariants = makeEnterVariants(12, 0.85);
const sublineEnterVariants = makeEnterVariants(10, 0.8);
const subcopyEnterVariants = makeEnterVariants(8, 0.8);
const ctaEnterVariants = makeEnterVariants(8, 0.75);
const activityEnterVariants = makeEnterVariants(10, 0.75);
const trustEnterVariants = makeEnterVariants(8, 0.75);
    ...
    return (
        ...
        >
            ...
            <div className="space-y-5">
                <motion.div variants={badgeEnterVariants}>
                    <motion.span
                        animate={floatEnabled ? { y: [0, -1.5, 0], opacity: [0.92, 1, 0.92] } : { y: 0, opacity: 1 }}
                        transition={
                            floatEnabled
                                ? {
                                      y: loopTransition(true, 6),
                                      opacity: loopTransition(true, 6),
                                  }
                                : { duration: 0.2 }
                        }
                ...
                <h1 className="flex flex-col items-center justify-center font-black text-white leading-[0.9] uppercase italic">
                    <motion.span
                        variants={lineEnterVariants}
                        className="block"
                    >
                        <motion.span
                            animate={floatEnabled ? { y: [0, -2.1, 0] } : { y: 0 }}
                            transition={loopTransition(floatEnabled, 5.4)}
                    ...
                    >
                        <motion.span
                            animate={floatEnabled ? { y: [0, -2.5, 0] } : { y: 0 }}
                            transition={loopTransition(floatEnabled, 5.9, 0.18)}
                            className={HERO_LINE_CLASS}
                        >
                            due diligence
                        </motion.span>
                    ...
                    <motion.span
                        variants={sublineEnterVariants}
                        className="mt-6 text-xl sm:text-3xl md:text-4xl font-bold text-accent tracking-wider relative block"
                    >
                        <motion.span
                            animate={floatEnabled ? { opacity: [0.9, 1, 0.9] } : { opacity: 1 }}
                            transition={loopTransition(floatEnabled, 4.1)}
                            className="block"
                        >
                            IN UNDER 2 MINUTES
                        </motion.span>
            ...
            {/* Subheading */}
            <motion.div variants={subcopyEnterVariants}>
                <motion.p
                    animate={floatEnabled ? { opacity: [0.9, 1, 0.9] } : { opacity: 1 }}
                    transition={loopTransition(floatEnabled, 7)}
            ...
            {/* CTA Buttons */}
            <motion.div variants={ctaEnterVariants} className="pt-2 sm:pt-3">
                <HeroActions />
            </motion.div>

            {/* Example Activity */}
            <motion.div
                variants={activityEnterVariants}
