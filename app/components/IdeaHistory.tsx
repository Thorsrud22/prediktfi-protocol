...
export default function IdeaHistory() {
    ...
    const filteredIdeas = ideas
        .filter(idea => {
            ...
            const matchesType = filterType === 'all' || idea.projectType === filterType;
    ...
    return (
        <div className="mt-16 space-y-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h2 className="text-2xl font-black tracking-tight text-white uppercase italic mb-2">
                        <span className="text-blue-500 mr-2">//</span> My Validated Ideas
                    </h2>
                    <p className="text-slate-400 text-sm">Review your past evaluations and track progress.</p>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full sm:w-48 bg-slate-900/60 border border-white/10 rounded-xl px-4 py-2 text-sm text-white placeholder-white/20 focus:outline-none focus:border-blue-500/50"
                        />
                    </div>

                    {/* Type Filter */}
                    <div className="flex p-1 bg-slate-900/60 border border-white/10 rounded-xl">
                        {['all', 'ai', 'memecoin', 'defi'].map((type) => (
                            <button
                                key={type}
                                onClick={() => setFilterType(type)}
                                className={`px-3 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${filterType === type
                    ...
                    <select
                        ...
                        className="bg-slate-900/60 border border-white/10 rounded-xl px-4 py-2 text-sm text-slate-300 focus:outline-none focus:border-blue-500/50 cursor-pointer appearance-none"
                    >
            ...
            {ideas.length === 0 ? (
                <div className="text-center py-16 bg-slate-900/40 rounded-2xl border border-slate-800 backdrop-blur-sm px-6">
                ...
                <div className="text-center py-12 border border-dashed border-white/10 rounded-2xl">
                    <p className="text-slate-500">No ideas found matching your filters.</p>
                    <button onClick={() => { setSearchTerm(''); setFilterType('all'); }} className="text-blue-400 text-sm font-bold mt-2 hover:underline">Clear Filters</button>
                </div>
            ) : (
                    ...
                    {hasMore && (
                        <div className="text-center pt-4">
                            <button
                                ...
                                className="group relative px-8 py-3 bg-slate-800/50 hover:bg-slate-800 border border-white/10 hover:border-blue-500/30 rounded-full transition-all"
                            >
                                <span className="text-xs font-bold text-slate-400 group-hover:text-white uppercase tracking-widest">
                                    Load More ({filteredIdeas.length - visibleCount})
                                </span>
                            </button>
                        </div>
                    )}
