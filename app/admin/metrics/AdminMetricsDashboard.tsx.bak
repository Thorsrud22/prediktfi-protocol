    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {Object.entries(metrics.resolution.resolverBreakdown).map(([resolver, data]) => (
            <div key={resolver} className="text-center">
              <h4 className="font-medium text-gray-900 mb-2">{resolver}</h4>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-red-600 font-medium">{data.error}</span>
                </div>
                <div className="flex justify-between text-sm">
      ...
      {/* Error Breakdown */}
      {metrics.resolution.errorBreakdown.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Recent Errors</h3>
          <div className="space-y-3">
            {metrics.resolution.errorBreakdown.map((error, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                <div className="flex-1">
                  <p className="text-sm font-medium text-red-900">{error.error}</p>
                  <p className="text-xs text-red-600">Resolver: {error.resolver}</p>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
          <h4 className="text-sm font-medium text-gray-600 mb-2">D1 Retention</h4>
          <p className="text-3xl font-bold text-gray-900">
            {formatPercentage(metrics.retention.d1Retention)}
          </p>
        </div>
        
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
          <h4 className="text-sm font-medium text-gray-600 mb-2">D7 Retention</h4>
          <p className="text-3xl font-bold text-gray-900">
            {formatPercentage(metrics.retention.d7Retention)}
          </p>
        </div>
        
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 text-center">
          <h4 className="text-sm font-medium text-gray-600 mb-2">Sharing Rate</h4>
          <p className="text-3xl font-bold text-gray-900">
            {formatPercentage(metrics.retention.sharingRate)}
          </p>
        </div>
      </div>
    </div>
